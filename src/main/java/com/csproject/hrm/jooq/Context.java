...
public class Context {
  ...
  private List<QueryFilter> filters(String filter) {
    ...
    return Arrays.stream(filter.split(COMMA))
        .map(
            each -> {
              ...
              return new QueryFilter(
                  split[ZERO_NUMBER], URLDecoder.decode(split[ONE_NUMBER], StandardCharsets.UTF_8));
            })
  ...
  private List<OrderByClause> orderBy(String orderBy) {
    ...
    return Arrays.stream(orderBy.split(COMMA))
        .map(
            each -> {
