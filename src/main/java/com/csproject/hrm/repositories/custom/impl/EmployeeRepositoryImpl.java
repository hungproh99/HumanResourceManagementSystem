...
public class EmployeeRepositoryImpl implements EmployeeRepositoryCustom {
	...
	@Override
	public void insertEmployee(HrmRequest hrmRequest, String employeeId, String companyEmail, String password,
	                           String workStatus, String companyName, String contractStatus, LocalDate startDate) {
		//		final DSLContext dslContext = DSL.using(connection.getConnection());
		//		dslContext.transaction(configuration->{
		//			DSLContext ctx = DSL.using(configuration);
		//			EmployeeRecord employeeRecord = ctx.insertInto(EMPLOYEE, EMPLOYEE.EMPLOYEE_ID, EMPLOYEE.FULL_NAME,
		//			                                               EMPLOYEE.COMPANY_EMAIL, EMPLOYEE.WORK_STATUS,
		//			                                               EMPLOYEE.PHONE_NUMBER, EMPLOYEE.GENDER, EMPLOYEE.BIRTH_DATE,
		//			                                               EMPLOYEE.ROLE_TYPE).values(employeeId,
		//			                                                                          hrmRequest.getFullName(),
		//			                                                                          companyEmail, workStatus,
		//			                                                                          hrmRequest.getPhone(),
		//			                                                                          hrmRequest.getGender(),
		//			                                                                          hrmRequest.getBirthDate(),
		//			                                                                          ERole.of(hrmRequest.getRole()))
		//			                                   .returning(EMPLOYEE.EMPLOYEE_ID).fetchOne();
		//
		//			WorkingContractRecord workingContractRecord = ctx.insertInto(WORKING_CONTRACT, WORKING_CONTRACT.EMPLOYEE_ID,
		//			                                                             WORKING_CONTRACT.BASE_SALARY,
		//			                                                             WORKING_CONTRACT.COMPANY_NAME,
		//			                                                             WORKING_CONTRACT.START_DATE,
		//			                                                             WORKING_CONTRACT.STATUS,
		//			                                                             WORKING_CONTRACT.TYPE_ID).values(employeeId,
		//			                                                                                              hrmRequest.getBaseSalary(),
		//			                                                                                              companyName,
		//			                                                                                              startDate,
		//			                                                                                              contractStatus,
		//			                                                                                              EContractType.of(
		//					                                                                                              hrmRequest.getContractName()))
		//			                                                 .returning().fetchOne();
		//
		//			ctx.insertInto(WORKING_PLACE, WORKING_PLACE.WORKING_CONTRACT_ID, WORKING_PLACE.AREA_ID,
		//			               WORKING_PLACE.JOB_ID, WORKING_PLACE.OFFICE_ID).values(
		//					workingContractRecord.getWorkingContractId(), EArea.of(hrmRequest.getArea()),
		//					EJob.of(hrmRequest.getJob()), EOffice.of(hrmRequest.getOffice())).execute();
		//		});
	}
	
	public int countEmployeeSameStartName(String standForName) {
		final DSLContext dslContext = DSL.using(connection.getConnection());
		final var query = dslContext.selectCount().from(EMPLOYEE).where(
				condition("{0} REGEXP {1}", EMPLOYEE.EMPLOYEE_ID, val("^" + standForName + "[0-9]+$")));
	...
	public int countAllEmployeeByCondition(List<Condition> conditions, List<OrderField<?>> sortFields,
	                                       Pagination pagination) {
		final DSLContext dslContext = DSL.using(connection.getConnection());
		final var query = dslContext.selectCount().from(EMPLOYEE).leftJoin(WORKING_CONTRACT).on(
	...
	public List<HrmResponse> findAllEmployee(List<Condition> conditions, List<OrderField<?>> sortFields,
		...
		final var query = dslContext.select(EMPLOYEE.EMPLOYEE_ID, EMPLOYEE.FULL_NAME, EMPLOYEE.COMPANY_EMAIL.as(EMAIL),
		                                    EMPLOYEE.WORK_STATUS, EMPLOYEE.PHONE_NUMBER.as(PHONE), when(
						                            EMPLOYEE.GENDER.eq(FEMALE), FEMALE).when(EMPLOYEE.GENDER.eq(MALE), MALE).as(GENDER),
		                                    EMPLOYEE.BIRTH_DATE, JOB.TITLE.as(JOB_NAME), OFFICE.NAME.as(OFFICE_NAME),
