...
public class EmployeeRepositoryImpl implements EmployeeRepositoryCustom {
    ...
    public void insertEmployee(HrmRequest hrmRequest, String employeeId, String companyEmail, String password,
        ...
        dslContext.transaction(configuration -> {
            ...
            EmployeeRecord employeeRecord = ctx.insertInto(EMPLOYEE,
            ...
            WorkingContractRecord workingContractRecord = ctx.insertInto(WORKING_CONTRACT,
        ...
        final var query =
                dslContext
                        ...
                        .where(condition("{0} REGEXP {1}",
                                EMPLOYEE.EMPLOYEE_ID, val("^" + standForName + "[0-9]+$")));
    ...
    public int countAllEmployeeByCondition(List<Condition> conditions, List<OrderField<?>> sortFields, Pagination pagination) {
        final DSLContext dslContext = DSL.using(connection.getConnection());
        final var query =
                dslContext
                        .selectCount()
                        .from(EMPLOYEE)
                        .leftJoin(WORKING_CONTRACT).on(WORKING_CONTRACT.EMPLOYEE_ID.eq(EMPLOYEE.EMPLOYEE_ID))
                        .leftJoin(CONTRACT_TYPE).on(CONTRACT_TYPE.TYPE_ID.eq(WORKING_CONTRACT.TYPE_ID))
                        .leftJoin(WORKING_PLACE).on(WORKING_PLACE.WORKING_CONTRACT_ID.eq(WORKING_CONTRACT.WORKING_CONTRACT_ID))
                        .leftJoin(AREA).on(AREA.AREA_ID.eq(WORKING_PLACE.AREA_ID))
                        .leftJoin(OFFICE).on(OFFICE.OFFICE_ID.eq(WORKING_PLACE.OFFICE_ID))
                        .leftJoin(JOB).on(JOB.JOB_ID.eq(WORKING_PLACE.JOB_ID))
                        .where(conditions)
                        .orderBy(sortFields)
    ...
    public List<HrmResponse> findAllEmployee(List<Condition> conditions, List<OrderField<?>> sortFields, Pagination pagination) {
        final DSLContext dslContext = DSL.using(connection.getConnection());
        final var query =
                dslContext
                        .select(
                                ...
                                when(EMPLOYEE.GENDER.eq(FEMALE), FEMALE)
                                        .when(EMPLOYEE.GENDER.eq(MALE), MALE).as(GENDER),
                                EMPLOYEE.BIRTH_DATE,
                                JOB.TITLE.as(JOB_NAME),
                                OFFICE.NAME.as(OFFICE_NAME),
                                AREA.NAME.as(AREA_NAME),
                                CONTRACT_TYPE.NAME.as(CONTRACT),
                                year(currentDate()).minus(year(WORKING_CONTRACT.START_DATE))
                                        .concat(YEAR)
